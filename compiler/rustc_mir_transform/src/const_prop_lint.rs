...
impl<'tcx> MirLint<'tcx> for ConstProp {
    fn run_lint(&self, tcx: TyCtxt<'tcx>, body: &Body<'tcx>) {
        ...
        if mutate_condition!(body.source.promoted.is_some(), 72) {
            return;
        }
        ...
        if mutate_condition!(!is_fn_like && !is_assoc_const, 73) {
        ...
        if mutate_condition!(is_generator, 74) {
        ...
        let predicates = tcx
            ...
            .predicates
            .iter()
            .filter_map(|(p, _)| if mutate_condition!(p.is_global(), 75) { Some(*p) } else { None });
...
impl<'mir, 'tcx> interpret::Machine<'mir, 'tcx> for ConstPropMachine<'mir, 'tcx> {
    ...
    fn access_local(
        ...
        if mutate_condition!(l.value == LocalValue::Unallocated, 76) {
    ...
    {
        if mutate_condition!(ecx.machine.can_const_prop[local] == ConstPropMode::NoPropagation, 77) {
            throw_machine_stop_str!("tried to write to a local that is marked as not propagatable")
        }
        if mutate_condition!(frame == 0 && ecx.machine.only_propagate_inside_block_locals.contains(local), 78) {
            trace!(
    ...
    fn before_access_global(
        ...
        if mutate_condition!(is_write, 79) {
            throw_machine_stop_str!("can't write to global");
        }
        // If the static allocation is mutable, then we can't const prop it as its content
        // might be different at runtime.
        if mutate_condition!(alloc.inner().mutability == Mutability::Mut, 80) {
...
impl<'mir, 'tcx> ConstPropagator<'mir, 'tcx> {
    fn new(
        ...
        for (l, mode) in can_const_prop.iter_enumerated() {
            if mutate_condition!(*mode == ConstPropMode::OnlyInsideOwnBlock, 81) {
    ...
    fn eval_constant(&mut self, c: &Constant<'tcx>, source_info: SourceInfo) -> Option<OpTy<'tcx>> {
        ...
        if mutate_condition!(c.needs_subst(), 82) {
            return None;
        }
        ...
        match self.ecx.mir_const_to_op(&c.literal, None) {
            ...
            Err(error) => {
                ...
                if let Some(lint_root) = self.lint_root(source_info) {
                    ...
                    if mutate_condition!(lint_only, 83) {
                        // Out of backwards compatibility we cannot report hard errors in unused
                        // generic functions using associated constants of the generic parameters.
    ...
    fn check_binary_op(
        ...
        if mutate_condition!(op == BinOp::Shr || op == BinOp::Shl, 84) {
            let r = r?;
            ...
            if mutate_condition!(r_bits.map_or(false, |b| b >= left_size.bits() as u128), 85) {
    ...
    fn const_prop(
        ...
        // FIXME we need to revisit this for #67176
        if mutate_condition!(rvalue.needs_subst(), 87) {
            return None;
        }
...
impl CanConstProp {
    ...
    fn check<'tcx>(
        ...
        for (local, val) in cpv.can_const_prop.iter_enumerated_mut() {
            ...
            // Cannot use locals because if mutate_condition!(x < y, 89) { y - x } else { x - y } would
            //        lint for x != y
            // FIXME(oli-obk): lint variables until they are used in a condition
            // FIXME(oli-obk): lint if return value is constant
            if mutate_condition!(cpv.local_kinds[local] == LocalKind::Arg, 90) {
                *val = ConstPropMode::OnlyPropagateInto;
                trace!(
                    "local {:?} can't be const propagated because it's a function argument",
                    local
                );
            } else if mutate_condition!(cpv.local_kinds[local] == LocalKind::Var, 91) {
    ...
    fn visit_local(&mut self, &local: &Local, context: PlaceContext, _: Location) {
        ...
        match context {
            ...
            | MutatingUse(MutatingUseContext::Store) => {
                if mutate_condition!(!self.found_assignment.insert(local), 92) {
...
impl<'tcx> Visitor<'tcx> for ConstPropagator<'_, 'tcx> {
            ...
            if let Some(()) = self.const_prop(rval, source_info, place) {
                match can_const_prop {
                    ...
                    ConstPropMode::OnlyPropagateInto | ConstPropMode::NoPropagation => {
                        ...
                        if mutate_condition!(place.local != RETURN_PLACE, 93) {
                            Self::remove_const(&mut self.ecx, place.local);
                        }
                    }
                ...
                StatementKind::SetDiscriminant { ref place, .. } => {
                    match self.ecx.machine.can_const_prop[place.local] {
                        ConstPropMode::FullConstProp | ConstPropMode::OnlyInsideOwnBlock => {
                            if mutate_condition!(self.use_ecx(|this| this.ecx.statement(statement)).is_some(), 94) {
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        ...
        match &terminator.kind {
            TerminatorKind::Assert { expected, ref msg, ref cond, .. } => {
                if let Some(ref value) = self.eval_operand(&cond, source_info) {
                    ...
                    if mutate_condition!(expected != value_const, 95) {
                        enum DbgVal<T> {
                            Val(T),
                            Underscore,
                        }
        ...
        if mutate_condition!(cfg!(debug_assertions), 96) {
